...
-- Autor         : MIGOMEZ
-- Fecha         : 19/01/2026
-- Modificaciones: ajuste generacion por grupo de contailidades 
----------------------------------------------------------------
create or replace PACKAGE Ap_fina_ConSaldosAllianzPkg AS

chrProceName_g MPDAF_.MPDAF__NOMBPROC__B%TYPE := 'Ap_fina_ConSaldosAllianzPkg';
chrVariables_g MPDAF_.MPDAF__MENSAJE___B%TYPE;
intNumMenReg_g TERCER.TERCER_IDENTIFIC_B%TYPE := 10;
chrProceInvo_g MPDAF_.MPDAF__NOMBPROG__B%TYPE := TO_CHAR((current_timestamp), 'YYYY-MM-DD HH:MM:SS AM');
TYPE arrConSaldos_p is TABLE OF VARCHAR2(250) INDEX BY PLS_INTEGER;
...
---------------------------------------------------------------
-- Invoca el proceso CalcularSaldoCuenta y retorna los saldos
-- inicial, final, d¿bitos y cr¿ditos en moneda local y origen.
--
-- El formato en que se devuelven los resultados es:
-- <IL>X1</IL><DL>X2</DL><CL>X3</CL><FL>X4</FL><IO>X5</IO><DO>X6</DO><CO>X7</CO><FO>X8</FO>
--
-- Donde:
-- X1: Saldo inicial en moneda local
-- X2: D¿bitos en moneda local
-- X3: Cr¿ditos en moneda local
-- X4: Saldo final en moneda local
-- X5: Saldo inicial en moneda origen
-- X6: D¿bitos en moneda origen
-- X7: Cr¿ditos en moneda origen
-- X8: Saldo final en moneda origen
--
-- Par metros de entrada
-- chrCodiCont__p C¢digo de contabilidad
-- chrCodiCuen__p C¢digo de la cuenta
-- chrFechSald__p Fecha del saldo requerido
-- chrInclAjus__p Indicador de si incluye ajustes
-- chrInclCier__p Indicador de si incluye cierres
-- chrCodCenUti_p C¢digo centro de utilidad
-- chrIndIncArb_p Indicador de si incluye arbol de centros
-- intIdenTerc__p Identificaci¢n del tercero
--
-- Valor retornado:
-- dblSalFinLoc_p Saldo final en moneda local
----------------------------------------------------------------


chrCodiCont__p IN SPC___.SPC____CODIGO____CONTAB_B%TYPE, -- C¢digo de contabilidad
...
chrCodCenUti_p IN SPCCU_.SPCCU__CODIGO____CU_____B%TYPE, -- C¢digo de centro de utilidad
chrIncArbCen_p IN VARCHAR2,                              -- Indicador de si incluye arbol de centros
intIdenTerc__p IN SPCT__.SPCT___IDENTIFIC_TERCER_B%TYPE  -- Identificaci?n del tercero
) RETURN VARCHAR2;

PROCEDURE CalcularSaldoCuenta
---------------------------------------------------------------
-- Calcula el saldo de una cuenta para un determinado per¡odo.
--
-- Par metros de entrada
-- chrCodiCont__p C¢digo de contabilidad
-- chrCodiCuen__p C¢digo de la cuenta
...
-- dblSalIniLoc_p Saldo inicial en moneda local
-- dblSalIniOri_p Saldo inicial en moneda origen
-- dblDebiLoca__p Movimiento d¿bito en moneda local
-- dblCredLoca__p Movimiento cr¿dito en moneda local
-- dblDebiOrig__p Movimiento d¿bito en moneda origen
-- dblCredOrig__p Movimiento cr¿dito en moneda origen
-- dblSalFinLoc_p Saldo final en moneda local
-- dblSalFinOri_p Saldo final en moneda origen
-- chrIndHijSal_p Indicador de si las cuentas hijas tuvieron saldo
----------------------------------------------------------------
( chrCodiCont__p IN SPC___.SPC____CODIGO____CONTAB_B%TYPE, -- C¢digo de contabilidad
  chrCodiCuen__p IN SPC___.SPC____CODIGO____CPC____B%TYPE, -- Cuenta
  ...
  dblDebiLoca__p OUT SPC___.SPC____SALINILOC_B%TYPE,       -- Movimiento d¿bito en moneda local
  dblCredLoca__p OUT SPC___.SPC____SALINILOC_B%TYPE,       -- Movimiento cr¿dito en moneda local
  dblDebiOrig__p OUT SPC___.SPC____SALINILOC_B%TYPE,       -- Movimiento d¿bito en moneda origen
  dblCredOrig__p OUT SPC___.SPC____SALINILOC_B%TYPE,       -- Movimiento cr¿dito en moneda origen
  ...
  chrIndHijSal_p OUT VARCHAR2                              -- Indicador de si las cuentas hijas tuvieron saldo
);
      ...
      SELECT CPC____CODIGO____B CUENTA, CPC____NOMBRE____B NOMBRE,
        ...
        FROM ( SELECT CPC____CODIGO____B, CPC____NOMBRE____B,
                ...
                WHERE CONTAB_CODIGO____PC_____B = CPC____CODIGO____PC_____B
                  ...
                  AND LENGTH(CPC____CODIGO____B) <= 6 )
             )
...
PROCEDURE Ap_fina_ConSaldos2 (
  ...
  chrCuentasPV_t PARAME.PARAME_VALOR_____B%TYPE;
  chrCPCposiPV_t PARAME.PARAME_VALOR_____B%TYPE;
  chrCPCelimPV_t PARAME.PARAME_VALOR_____B%TYPE;

  chrTextSele__t VARCHAR2(30000);   --SQL dinámico
  chrCodiCont__t MC____.MC_____CODIGO____CONTAB_B%TYPE;
  chrFecSystem_t MC____.MC_____FECHA_____B%TYPE;
  
  CURSOR curElimCuentas__t IS
   SELECT SCALL__CODIGO____CPC____B, SCALL__MONEDA_CODIGO____B
     FROM SCALL_
    WHERE SCALL__MONEDA_CODIGO____B IS NOT NULL
      ;
...
BEGIN
   ...
   /* SE ACTUALIZA POR REQUERIMIENTO 15370 MIGOMEZ
   BEGIN
   ...
   SELECT  GC_____COGEBANA__B 
   INTO chrCodiCont__t
   FROM GC____,GCC___
   WHERE GC_____CODIGO____B = GCC____CODIGO____GC_____B
   AND GCC____CODIGO____CONTAB_B = chrcodicont__p;
   ...
   BEGIN
      SELECT PARAME_VALOR_____B
        ...
        FROM PARAME
       WHERE PARAME_CODIGO____SISTEM_B = 'FINA'
         AND PARAME_NUMERO____B = 92;
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
          chrCadeErro__p := chr(13) || chrcadeerro__p ||' ERROR. Parámetro para cuentas PV cambio moneda. No Existe';
   END;
   ...
   BEGIN
      SELECT PARAME_VALOR_____B
        ...
        FROM PARAME
       WHERE PARAME_CODIGO____SISTEM_B = 'FINA'
         AND PARAME_NUMERO____B = 93;
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
          chrCadeErro__p := chr(13) || chrcadeerro__p ||' ERROR. Parámetro para cuentas PV cambio signo. No Existe';
   END;
   ...
   BEGIN
      SELECT PARAME_VALOR_____B
        ...
        FROM PARAME
       WHERE PARAME_CODIGO____SISTEM_B = 'FINA'
         AND PARAME_NUMERO____B = 94;
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
          chrCadeErro__p := chr(13) || chrcadeerro__p ||' ERROR. Parámetro para cuentas PV a eliminar. No Existe';
   END;
   ...
   IF( chrIndValExi_p = 'S' ) THEN
      IF( NVL(intSecuInte__p, -999) <= 0 ) THEN
         chrIndValExi_p := 'N';
         chrCadeErro__p := chr(13) || chrcadeerro__p || 'CONSECUTIVO NO VÿLIDO';
      ELSE
         ...
         IF( chrCodiCont__t = chrCodiCont__p ) THEN
            FOR regConsCuentas__t IN curConsCuentas__t LOOP
               ...
               chrVariables_g := ' 243 InsertarTemporal '|| chrTextSele__t;
         ...
         ELSE
            FOR regConsCuentaI__t IN curConsCuentaI__t LOOP
               ...
               chrVariables_g := ' 277 InsertarTemporal '|| chrTextSele__t;
        ...
        END IF;  
         ...
         IF NVL(chrCPCposiPV_t, 'XoXoXo') <> 'XoXoXo'  THEN
            chrTextSele__t := 'UPDATE SCALL_ SET SCALL__SALFINLOC_B = -1 * SCALL__SALFINLOC_B WHERE SCALL__SALFINLOC_B < 0 AND SCALL__CODIGO____CPC____B IN '||chrCPCposiPV_t;
            EXECUTE IMMEDIATE chrTextSele__t;
         END IF;
         
         IF NVL(chrCuentasPV_t, 'XoXoXo') <> 'XoXoXo'  THEN   
            chrTextSele__t := 'UPDATE SCALL_ SET SCALL__MONEDA_CODIGO____B = '||CHR(39)||'USD'||CHR(39)||' WHERE SCALL__CODIGO____CPC____B IN '||chrCuentasPV_t;
            EXECUTE IMMEDIATE chrTextSele__t;
         END IF;
   ...
   IF( chrIndValExi_p = 'S' ) THEN
      FOR regElimCuentas__t IN curElimCuentas__t LOOP
         BEGIN
             DELETE FROM SCALL_
             WHERE regElimCuentas__t.SCALL__CODIGO____CPC____B = RPAD( SCALL__CODIGO____CPC____B, 6, '0')
               AND NVL(SCALL__MONEDA_CODIGO____B, 'XoXo') = 'XoXo';
          EXCEPTION     
             WHEN OTHERS THEN
                RAISE_APPLICATION_ERROR(-20050, 'Error eliminando registros duplicados');             
         END;
      END LOOP;
   END IF;
...
begin
   UPDATE SCALL_
   SET SCALL__CODIGO____CPC____B = '0'||SCALL__CODIGO____CPC____B;
   EXCEPTION     
      WHEN OTHERS THEN
         RAISE_APPLICATION_ERROR(-20050, 'Error actualizando registros CPC '||SQLERRM);             
END;

INSERT INTO SCALL_ (SCALL__SECINTDOC_B,SCALL__CODIGO____CPC____B,SCALL__CPC____NOMBRE____B,SCALL__SALINILOC_B,SCALL__ACUDEBLOC_B,SCALL__ACUCRELOC_B,SCALL__SALFINLOC_B,SCALL__FECHORCRE_B,SCALL__AUTOCREA__B,SCALL__MONEDA_CODIGO____B)
SELECT SCALL__SECINTDOC_B,
'1'||SUBSTR(SCALL__CODIGO____CPC____B,2,6),
SCALL__CPC____NOMBRE____B,
SCALL__SALINILOC_B,
SCALL__ACUDEBLOC_B,
SCALL__ACUCRELOC_B,
SCALL__SALFINLOC_B,
...
SCALL__MONEDA_CODIGO____B
FROM SCALL_ 
WHERE SCALL__MONEDA_CODIGO____B = 'COP'  --GENE 6
AND SUBSTR(SCALL__CODIGO____CPC____B,1,1) = '0';

INSERT INTO SCALL_ (SCALL__SECINTDOC_B,SCALL__CODIGO____CPC____B,SCALL__CPC____NOMBRE____B,SCALL__SALINILOC_B,SCALL__ACUDEBLOC_B,SCALL__ACUCRELOC_B,SCALL__SALFINLOC_B,SCALL__FECHORCRE_B,SCALL__AUTOCREA__B,SCALL__MONEDA_CODIGO____B)
SELECT SCALL__SECINTDOC_B,
'2'||SUBSTR(SCALL__CODIGO____CPC____B,2,6),
SCALL__CPC____NOMBRE____B,
SCALL__SALINILOC_B,
SCALL__ACUDEBLOC_B,
SCALL__ACUCRELOC_B,
...
INSERT INTO SCALL_ (SCALL__SECINTDOC_B,SCALL__CODIGO____CPC____B,SCALL__CPC____NOMBRE____B,SCALL__SALINILOC_B,SCALL__ACUDEBLOC_B,SCALL__ACUCRELOC_B,SCALL__SALFINLOC_B,SCALL__FECHORCRE_B,SCALL__AUTOCREA__B,SCALL__MONEDA_CODIGO____B)
SELECT SCALL__SECINTDOC_B,
SUBSTR(SCALL__CODIGO____CPC____B,1,5),
NULL,
SUM(SCALL__SALINILOC_B),
SUM(SCALL__ACUDEBLOC_B),
SUM(SCALL__ACUCRELOC_B),
SUM(SCALL__SALFINLOC_B),
SYSDATE,
USER,
NULL
FROM SCALL_ 
WHERE SUBSTR(SCALL__CODIGO____CPC____B,1,1) = '2'
AND LENGTH(SCALL__CODIGO____CPC____B) = 7
GROUP BY SCALL__SECINTDOC_B, SUBSTR(SCALL__CODIGO____CPC____B,1,5);

INSERT INTO SCALL_ (SCALL__SECINTDOC_B,SCALL__CODIGO____CPC____B,SCALL__CPC____NOMBRE____B,SCALL__SALINILOC_B,SCALL__ACUDEBLOC_B,SCALL__ACUCRELOC_B,SCALL__SALFINLOC_B,SCALL__FECHORCRE_B,SCALL__AUTOCREA__B,SCALL__MONEDA_CODIGO____B)
SELECT SCALL__SECINTDOC_B,
SUBSTR(SCALL__CODIGO____CPC____B,1,3),
NULL,
SUM(SCALL__SALINILOC_B),
SUM(SCALL__ACUDEBLOC_B),
SUM(SCALL__ACUCRELOC_B),
...
WHERE SUBSTR(SCALL__CODIGO____CPC____B,1,1) = '2'
AND LENGTH(SCALL__CODIGO____CPC____B) = 7
GROUP BY SCALL__SECINTDOC_B, SUBSTR(SCALL__CODIGO____CPC____B,1,3);

INSERT INTO SCALL_ (SCALL__SECINTDOC_B,SCALL__CODIGO____CPC____B,SCALL__CPC____NOMBRE____B,SCALL__SALINILOC_B,SCALL__ACUDEBLOC_B,SCALL__ACUCRELOC_B,SCALL__SALFINLOC_B,SCALL__FECHORCRE_B,SCALL__AUTOCREA__B,SCALL__MONEDA_CODIGO____B)
SELECT SCALL__SECINTDOC_B,
SUBSTR(SCALL__CODIGO____CPC____B,1,2),
NULL,
SUM(SCALL__SALINILOC_B),
SUM(SCALL__ACUDEBLOC_B),
SUM(SCALL__ACUCRELOC_B),
...
WHERE SUBSTR(SCALL__CODIGO____CPC____B,1,1) = '2'
AND LENGTH(SCALL__CODIGO____CPC____B) = 7
GROUP BY SCALL__SECINTDOC_B, SUBSTR(SCALL__CODIGO____CPC____B,1,2);
...
NULL
FROM SCALL_ 
WHERE SUBSTR(SCALL__CODIGO____CPC____B,1,1) = '1'
AND LENGTH(SCALL__CODIGO____CPC____B) = 7
GROUP BY SCALL__SECINTDOC_B, SUBSTR(SCALL__CODIGO____CPC____B,1,5);
...
--******************************************************************
   IF( chrIndValExi_p = 'S' ) THEN
      FOR regElimCuentas__t IN curElimCuentas__t LOOP
         BEGIN
             DELETE FROM SCALL_
             WHERE SUBSTR(regElimCuentas__t.SCALL__CODIGO____CPC____B,2,6) = RPAD( SUBSTR(SCALL__CODIGO____CPC____B,2, 6), 6, '0')
               AND NVL(SCALL__MONEDA_CODIGO____B, 'XoXo') = 'XoXo';
          EXCEPTION     
             WHEN OTHERS THEN
                RAISE_APPLICATION_ERROR(-20050, 'Error eliminando registros duplicados');             
         END;
      END LOOP;
      IF NVL(chrCPCelimPV_t, 'XoXoXo') <> 'XoXoXo'  THEN
         chrTextSele__t := 'DELETE FROM SCALL_ WHERE LENGTH(SCALL__CODIGO____CPC____B) = 2 AND SUBSTR(SCALL__CODIGO____CPC____B,2,1) IN '||chrCPCelimPV_t;
         EXECUTE IMMEDIATE chrTextSele__t;
      END IF;
   END IF;
...
intUltPerSal_t PERIOD.PERIOD_NUMERO____B%TYPE;         -- ÿltimo período a tomar de saldos
...
SELECT '107677', --SPC____CODIGO____CONTAB_B,
       ...
       CAC____AJUDEB12__B,
       CAC____AJUCRE12__B
FROM SPC___, CPC___, CAC___,GCC___
WHERE SPC____CODIGO____CONTAB_B = CAC____CODIGO____CONTAB_B
  ...
  AND SPC____CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)
...
FROM SPCCU_, CACCU_, CPC___, GCC___
WHERE SPCCU__CODIGO____CONTAB_B = CACCU__CODIGO____CONTAB_B
  ...
  AND SPCCU__CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)
...
FROM SPCT__, CACT__, CPC___,GCC___
WHERE SPCT___CODIGO____CONTAB_B = CACT___CODIGO____CONTAB_B
  ...
  AND SPCT___CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)
...
FROM SPCCUT, CACCUT, CPC___,GCC___
WHERE SPCCUT_CODIGO____CONTAB_B = CACCUT_CODIGO____CONTAB_B
  ...
  AND SPCCUT_CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)
...
WHERE --MC_____CODIGO____CONTAB_B = chrCodiCont__p AND
  MC_____CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)
  AND    MC_____CODIGO____PF_____B = chrCodPerFis_t
...
WHERE --MC_____CODIGO____CONTAB_B = chrCodiCont__p AND
  ...
  AND MC_____CODIGO____CONTAB_B = GCC____CODIGO____CONTAB_B --AJUSTE 15370 MIGOMEZ SE TRAE EL GRUPO DE CONTABILIDAD
  AND GCC____CODIGO____GC_____B = (SELECT DISTINCT GCC____CODIGO____GC_____B
                                   FROM GCC___
                                   WHERE GCC____CODIGO____CONTAB_B = chrCodiCont__p)  
  AND MC_____CODIGO____CPC____B BETWEEN chrCodiCuen__p AND RPAD(chrCodiCuen__p,26,'Z')
...
BEGIN
  ...
  IF chrCodPlaCue_g IS NULL THEN
    ...
    EXCEPTION
      ...
      WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20050, 'AQUÿ|');
    END;
  ELSE
    -- Req. 4824
