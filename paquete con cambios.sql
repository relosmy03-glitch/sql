...
chrCodiCLIE__g  PARAME.PARAME_VALOR_____B%TYPE;--CODIGO CLIENTE
   ...
   AND GCC____CODIGO____CONTAB_B = chrcodicont__p;
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
          --chrCadeErro__p := chr(13) || chrcadeerro__p ||' ERROR. ParÃ¡metro para contabilidad por defecto NO EXISTE';
          chrCodiCont__t := chrcodicont__p;
   END;
--DBMS_OUTPUT.PUT_LINE(' ***chrcodicont__p = ' || chrcodicont__p);
   BEGIN
      SELECT PARAME_VALOR_____B
        INTO chrCuentasPV_t
        FROM PARAME
    ...
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
          chrCadeErro__p := chr(13) || chrcadeerro__p ||' ERROR. ParÃ¡metro para cuentas PV cambio moneda. No Existe';
   END;
   BEGIN
      SELECT PARAME_VALOR_____B
        INTO chrCodiCLIE__g
        FROM PARAME
       WHERE PARAME_CODIGO____SISTEM_B = 'SEGU'
         AND PARAME_NUMERO____B = 11;
    EXCEPTION
       WHEN NO_DATA_FOUND THEN
          chrIndValExi_p := 'N';
...
chrResultado_t VARCHAR2(400);
 cursor curgrupcont__t is
 select GCC____CODIGO____CONTAB_B from gcc___,CONTAB
 where GCC____CODIGO____GC_____B in ( select GCC____CODIGO____GC_____B from gcc___
                                       where  GCC____CODIGO____CONTAB_B = chrcodicont__p)
  AND CONTAB_CODIGO____B = GCC____CODIGO____CONTAB_B;
...
BEGIN
  ...
  -- 
 -- DBMS_OUTPUT.PUT_LINE(' ***chrcodicont__p = ' || chrcodicont__p);
 --DBMS_OUTPUT.PUT_LINE(' ***chrcodiclie__g = ' || chrcodiclie__g);
  /*IF chrcodiclie__g = 'AL123' THEN
  
      FOR reggrupcont__t IN curgrupcont__t LOOP 
     --  DBMS_OUTPUT.PUT_LINE(' ***chrcodiclie__g = ' || reggrupcont__t.GCC____CODIGO____CONTAB_B);
      CalcularSaldoCuenta(reggrupcont__t.GCC____CODIGO____CONTAB_B,  -- Código de contabilidad
    ...
    END LOOP;                      
  ELSE*/
  AP_GENE_SEGUIMIENTO.RegistrarMensaje('saldos','loop 1 chrcodicont__p = '||chrcodicont__p||'chrCodiCuen__p'||chrCodiCuen__p);
  ...
  ELSE
    ...
    IF chrcodiclie__g = 'AL123' THEN
       ...
       FOR reggrupcont__t IN curgrupcont__t LOOP 
       AP_GENE_SEGUIMIENTO.RegistrarMensaje('saldos','loop 2 chrcodicont__p = '||reggrupcont__t.GCC____CODIGO____CONTAB_B||'chrCodiCuen__p'||chrCodiCuen__p);
        CalcularSaldoCuenta(reggrupcont__t.GCC____CODIGO____CONTAB_B,  -- Código de contabilidad
                            chrCodiCuen__p,  -- Cuenta
